...
async Task Main() {
    ...
    string estadoNaTela = "";
    string valorantigo = angulo("antigo");
    string valornovo = Convert.ToString(Bot.Compass);

	while(true) {
		if (estadoNaTela!=estado) {
			IO.PrintLine(estado);
			estadoNaTela = estado;
            IO.PrintLine(Convert.ToString(valorantigo));
            IO.PrintLine("valor do angulo novo"+valornovo);
		}
        if )estadoNaTel
        if (estado == "indo pra frente"){
            ...
            if(cor("cc")=="Preto" && cor("lc1")=="Preto" && cor("rc1")=="Preto"){
                motor("flmotor", 1000, 0);
                motor("frmotor", 1000, 0);
                motor("blmotor", 1000, 0);
                motor("brmotor", 1000, 0);
